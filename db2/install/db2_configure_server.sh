...
check_error
#workaround if db2 is not startet
ERR_MESSAGE=`su - $DB2I_USER -c "db2start 2>&1"`
#doing nothing on error, just ignore return value
echo -n "setup tcpip .. "
ERR_MESSAGE=`su - $DB2I_USER -c "db2set DB2COMM=tcpip 2>&1"`
check_error
echo -n "Stopping db2 .. "
ERR_MESSAGE=`su - $DB2I_USER -c "db2 db2stop" 2>&1`
check_error
echo -n "Starting db2 .. "
ERR_MESSAGE=`su - $DB2I_USER -c "db2 db2start 2>&1"`
check_error
